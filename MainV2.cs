...
namespace MissionPlanner
{
    ...
    public partial class MainV2 : Form
    {
        ...
        public abstract class menuicons
        {
            ...
            public abstract Image pocketball { get; }
        ...
        public class burntkermitmenuicons : menuicons
        {
            ...
            public override Image pocketball
            {
                get { return global::MissionPlanner.Properties.Resources.pocketball; }
            }
        ...
        public void updateLayout(object sender, EventArgs e)
        {
            ...
            OpenForm.Visible = DisplayConfiguration.displayOpenForm;
        ...
        public MainV2()
        {
            ...
            if (Program.Logo != null && Program.name == "VVVVZ")
            {
                OpenForm.Click -= this.OpenFrom_Click;
                OpenForm.Text = "";
                OpenForm.Image = Program.Logo;

                OpenForm.Click += MenuCustom_Click;
        ...
        public void switchicons(menuicons icons)
        {
            ...
            OpenForm.Image = displayicons.pocketball;
            ...
            OpenForm.ForeColor = ThemeManager.TextColor;
        }
        ...
        private void OpenFrom_Click(object sender, EventArgs e) {
            ...
            socket.Connect("140.119.164.19", 7274); // 1.設定 IP:Port 2.連線至伺服器
            ...
            socket.Close();
            //二維座標字串處理
            string text = System.IO.File.ReadAllText(@"Route\path.txt");
            char[] delimiterChars ={' ',',','[',']'};
            string[] words = text.Split(delimiterChars,System.StringSplitOptions.RemoveEmptyEntries);
            float[] numbers = Array.ConvertAll(words, new Converter<string, float>(StringToFloat));
            PointF[] points = new PointF[numbers.Length / 3];
            float[] height = new float[numbers.Length / 3];
            for (int i = 0; i < numbers.Length; i += 3)
            {
                points[i / 3].X = numbers[i]*100;
                points[i / 3].Y = numbers[i + 1]*100;
                height[i / 3] = numbers[i + 2];
            }
            Form2 form1 = new Form2();
            form1.Text = "Drawing";
            form1.ClientSize = new Size(981, 808);
            form1.FormBorderStyle = FormBorderStyle.FixedDialog;
            form1.StartPosition = FormStartPosition.CenterScreen;
            form1.Show();
            Graphics g = form1.CreateGraphics();
            Pen selPen = new Pen(Color.Black);
            Pen pathPen = new Pen(Color.Blue, 4);

            //來源影像
            string sourcePath = @"Resources\classroom.jpg";
            //取得來源影像
            Image souceImage = Image.FromFile(sourcePath);
            g.DrawImage(souceImage, 0, 0, 981, 808);
            ...
            for (int j = 1; j < 9; j++)
            {
                ...
                g.DrawLine(selPen,
                    0,
                    (100) * j,
                    form1.ClientSize.Width,
                    (100) * j);
            }
            //g.DrawRectangle(selPen, 10, 10, 150, 150);

            g.DrawLines(pathPen, points);

            //draw the starting point and end point
            int imagesize = 30;
            Image startPoint = Image.FromFile(@"Resources\start.png");
            Image endPoint = Image.FromFile(@"Resources\end.png");
            g.DrawImage(startPoint, points[0].X - (imagesize / 2), points[0].Y - (imagesize / 2), imagesize, imagesize);
            g.DrawImage(endPoint, points[points.Length - 1].X - (imagesize / 2), points[points.Length - 1].Y - (imagesize / 2), imagesize, imagesize);

            //draw the drone
            Image simdrone = Image.FromFile(@"Resources\01_05.png");
            g.DrawImage(simdrone, 250 - (imagesize / 2), 50 - (imagesize / 2), imagesize, imagesize);

            g.Dispose();

            //inputbox
            string value = "(0,0,0.5)";
            if (InputBox("Mission Start", "Initial position:", ref value) == DialogResult.OK)
            {
